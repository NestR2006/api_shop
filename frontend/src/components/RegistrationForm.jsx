...
const RegistrationForm = () => {
  ...
  const setMessage = (msg, status, color) => {
    setStatusAndMessage({ status: true, text: msg, color: color });

    setTimeout(() => {
      setStatusAndMessage({ status: false, text: msg, color: color });
    }, 3000);
  };
  ...
  const onSubmitHandler = async (e) => {
    ...
    if (passwordRef.current.value !== confirmPasswordRef.current.value) {
      setMessage("Пароли не совпадают", true, "#dd2929");
      return;
    }
    ...
    if (
      ...
      setMessage("Поля не заполнены", true, "#dd2929");
      return;
    }
    ...
    if (response.ok) {
      setMessage("Регистрация прошла успешно", true, "green");
    } else {
      console.log(data.detail);
      setMessage(data.detail, true, "red");
    }
  };
  ...
  return (
    <>
      <div
        className={
          "notification" +
          (notificationStatusAndMessage.status ? " active" : "")
        }
        style={{
